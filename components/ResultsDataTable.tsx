...
// export default function ResultsDataTable({ query, type, drawn = false }: { query: string; type: string; drawn?: boolean }) {
...
//   const fetchResults = useCallback(async (page: number, size: number) => {
...
//           },
//           body: JSON.stringify({ smiles: query, page, pageSize: size }),
//         });
//       } else {
...
//   useEffect(() => {
//     if (query || window.location.search.includes("advanced=true")) {
//       fetchResults(currentPage, pageSize);
//     }
//   }, [query, type, drawn, currentPage, pageSize, fetchResults]);
...
//     if (!event.api || isFetchingRef.current) return;

//     const newPage = event.api.paginationGetCurrentPage() + 1; // AG-Grid is 0-indexed
//     const newPageSize = event.api.paginationGetPageSize();

//     if (newPage !== currentPage || newPageSize !== pageSize) {
//       setCurrentPage(newPage);
//       setPageSize(newPageSize);
//       setLoading(true); // Set loading true before fetching new page
//       fetchResults(newPage, newPageSize);
//     }
//   }, [currentPage, pageSize, fetchResults]);

//   const handlePageSizeChange = (newSize: number) => {
//     setPageSize(newSize);
//     setCurrentPage(1);
//     setLoading(true); // Set loading true for page size change
...
//         .filter((col) => col.field !== "compound.structure" && columnVisibility[col.field || ""])
//         .map((col) => ({
//           id: col.field || "",
...
//         if (node.data) {
//           filteredData.push(node.data as RowData);
//         }
//       });
...
//           if (objectKey === "compound" && propertyKey && propertyKey !== "drugs") {
//             const compound = row.compound;
//             if (propertyKey === "meltingPoint") {
//               const mp = compound.meltingPoint;
//               if (mp) {
//                 if (mp.min && mp.max && mp.min !== mp.max) {
//                   value = `${mp.min} - ${mp.max}`;
//                 } else if (mp.min) {
//                   value = `${mp.min}`;
//                 } else if (mp.max) {
...
//                 value = "N/A";
//               }
//             } else if (propertyKey in compound) {
//               value = compound[propertyKey as keyof Compound] as string | number ?? "";
//             }
//           } else if (objectKey === "drug" && propertyKey) {
//             const drug = row.drug;
//             if (propertyKey in drug) {
//               value = drug[propertyKey as keyof Drug] ?? "";
//             }
//           }
...
//       const fileName = `API-Surrogate-${query}-results-${getFormattedDate()}.xlsx`;
...
//   // Show loader during initial load or pagination
//   // if (loading) {
//   //   return (
...
//   // Only show "No Results" after initial load and if totalResults is 0
//   if (totalResults === 0 && !isInitialLoad && !loading) {
//     return (
...
//   const maxSimilarity = results.reduce((max, item) => Math.max(max, item.similarityScore || 0), -Infinity);
//   const minSimilarity = results.reduce((min, item) => Math.min(min, item.similarityScore || Infinity), Infinity);
...
//           ) : (
//             <p className="text-muted-foreground">
//               {totalResults} total results ({totalDrugs} formulations for {components} API(s) on this page)
//             </p>
//           )}
//         </div>
...
//               className="h-9 px-3 py-1 text-sm rounded-md border border-input bg-background hover:bg-accent hover:text-accent-foreground focus:outline-none focus:ring-2 focus:ring-ring"
//             >
