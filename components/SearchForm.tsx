  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <form onSubmit={handleSearch} className="space-y-4">
        ...
        <div className={`bg-white/10 rounded-md p-4 ${isSmiles ? "opacity-50" : ""}`}>
          <button
            type="button"
            onClick={() => !isSmiles && setShowPropertySearch(!showPropertySearch)}
            className={`w-full flex items-center justify-between text-primary-foreground/90 hover:text-primary-foreground font-medium ${
              isSmiles ? "cursor-not-allowed" : "hover:text-primary-foreground"
            }`}
            disabled={isSmiles}
          >
            <span>Advanced search {isSmiles && ""}</span>
            {showPropertySearch && !isSmiles ? (
